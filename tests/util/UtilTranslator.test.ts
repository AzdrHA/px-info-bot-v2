...
describe('UtilTranslator', () => {
  it('should get exception for invalid file', () => {
    expect(() => translator('test', {}, 'invalid')).toThrowError(
  ...
  it('should the translated string', () => {
    const res = translator(
      'Translation key "{KEY}" not found in "{FILE}" file',
      {},
      'error'
    );
  ...
  it('should log a warning and return the key for an invalid key', () => {
    ...
    expect(UtilLogger.warn).toHaveBeenCalledWith(
      `Translation key "${key}" not found in file "message"`
    );
