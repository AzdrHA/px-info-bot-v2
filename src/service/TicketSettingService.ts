...
export default class TicketSettingService {
  ...
  public updateArchivedCategory = async (
    client: Client,
    category: CategoryChannel
  ): Promise<Message | boolean> => {
  ...
  public updateContentMessage = async (
    client: Client,
    content: string
  ): Promise<Message | boolean> => {
  ...
  private readonly __message = async (
    ticket: ITicketSetting
  ): Promise<BaseMessageOptions> => {
    return {
      content: ticket.content.replace(
        /\{CONTENT_BUTTON}/,
        ticket.contentButton
      ),
  ...
  private readonly __updateMessage = async (
    ...
    if (channel == null)
      channel = client.channels.cache.get(ticketSetting.channel) as TextChannel;
