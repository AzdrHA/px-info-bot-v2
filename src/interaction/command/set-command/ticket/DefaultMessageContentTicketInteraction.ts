...
export default class DefaultMessageContentTicketInteraction extends AbstractInteraction {
  ...
  public async run (): Promise<any> {
    ...
    await ticketSettingService.updateContentMessage(this.client, TicketSettingService.DEFAULT_MESSAGE_CONTENT)
    await this.success(translator('The **{TYPE}** has been successfully updated', { TYPE: translator('Message Content') }))
    UtilLogger.success('DefaultContentButtonVerificationInteraction callback ' + TicketSettingService.DEFAULT_MESSAGE_CONTENT)
  }
}
