...
export default class ArchivedCategoryTicketInteraction extends AbstractInteraction {
  ...
  public async run (): Promise<any> {
    ...
    const callback = async (content: string): Promise<void> => {
      const category = (this.interaction.guild != null) ? this.interaction.guild.channels.cache.get(getIdFromChannelMention(content)) : null
      if (category == null || !(category instanceof CategoryChannel)) throw new AppException('The value entered is invalid')
      ...
      await this.success(translator('The **{TYPE}** has been successfully updated', { TYPE: translator('Archived Category') }))
      UtilLogger.success('ArchivedCategoryTicketInteraction callback' + category.id)
    }

    return await this.messageButtonCollector(await this.send({
      content: translator('Send the ID or mention the **{TYPE}** you want to use', { TYPE: translator('Archived Category') }),
