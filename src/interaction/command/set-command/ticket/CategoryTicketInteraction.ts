...
export default class CategoryTicketInteraction extends AbstractInteraction {
  ...
  public async run (): Promise<any> {
    ...
    const callback = async (content: string): Promise<void> => {
      ...
      await this.success(translator('The **{TYPE}** has been successfully updated', { TYPE: translator('Category') }))
    ...
    return await this.messageButtonCollector(await this.send({
      content: translator('Send the ID or mention the **{TYPE}** you want to use', { TYPE: translator('Category') }),
