...
export default class DefaultContentButtonVerificationInteraction extends AbstractInteraction {
  ...
  public async run (): Promise<any> {
    ...
    await verificationSettingService.updateContentButton(this.client, VerificationSettingService.DEFAULT_BUTTON_CONTENT)
    await this.success(translator('The **{TYPE}** has been successfully updated', { TYPE: translator('Content Button') }))
    UtilLogger.success('DefaultContentButtonVerificationInteraction callback ' + VerificationSettingService.DEFAULT_BUTTON_CONTENT)
  }
}
