...
export default class ContentButtonVerificationInteraction extends AbstractInteraction {
  ...
  public async run(): Promise<any> {
    ...
    const callback = async (content: string): Promise<void> => {
      ...
      UtilLogger.success(
        'ContentButtonVerificationInteraction callback ' + content
      );
    };
    ...
    return await this.messageButtonCollector(
      await this.send({
        content: translator(
          'Send the new content of the verification **{TYPE}**',
          { TYPE: translator('Content Button') }
        ),
        components:
          await new ContentButtonVerificationSetCommandButton().buildButton()
      }),
      callback
    );
  }
