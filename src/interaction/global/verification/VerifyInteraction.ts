    ...
    if (
      ...
      if (this.interaction.member.roles.cache.has(memberRole)) {
        await this.interaction.reply({
          content: translator('You are already verified'),
          ephemeral: true
        });
      } else {
        this.interaction.member.roles
          ...
          .then(async () => {
            await this.interaction.reply({
              content: translator('You are now verified'),
              ephemeral: true
            });
          })
          .catch(async () => {
            await this.interaction.reply({
              content: translator(
                'I do not have the necessary authorizations to give you the member role'
              ),
              ephemeral: true
            });
          });
